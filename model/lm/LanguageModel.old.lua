...
function layer:__init(opt)
  ...
  elseif self.rnn_type == 'RHN' then
    self.core = RHN.rhn(self.input_encoding_size, self.vocab_size + 1, self.rnn_size, self.num_layers, dropout)
    self.state_num = 1
...
function layer:sample_beam_new(imgs, opt)
  local beam_size = utils.getopt(opt, 'beam_size', 1)
  local div_group = utils.getopt(opt,'div_group', 1)
  local lambda = utils.getopt(opt,'lambda',0.5)
  local batch_size, feat_dim = imgs:size(1), imgs:size(2)
  local function compare(a,b) return a.p > b.p end -- used downstream
  ...
  for k=1,batch_size do
    ...
    state_table = {}
    for i=1,div_group do
      state_table[i] = {}
    end
    beam_seq_table = {} -- table that stores the sequences
    for i=1,div_group do
      beam_seq_table[i] = torch.LongTensor(self.seq_length, bdash):zero()
    end
    beam_seq_logprobs_table = {} --table to store logprobs of sequences
    for i=1,div_group do
      beam_seq_logprobs_table[i] = torch.LongTensor(self.seq_length, bdash):zero()
    end
    beam_logprobs_sum_table = {} --table to store joint probability
    for i=1,div_group do
      beam_logprobs_sum_table[i] = torch.CudaTensor(bdash):fill(0)
    end
    ...
    done_beams_flag = {}
    for i=1,div_group do
      done_beams_flag[i] = torch.CudaTensor(bdash):fill(0)
    end
    state = {} -- init state with zeros
    for h =1,self. num_layers*2 do
      state[h] = torch.CudaTensor(bdash, self.rnn_size):fill(0)
    end
    for i=1,div_group do
      state_table[i] = net_utils.clone_list(state)
    end
    logprobs_table = {}
    for i=1,div_group do
      logprobs_table[i] = torch.zeros(bdash, self.vocab_size)
    end
