...
function CompactBilinearPooling:reset()
   self.h1 = torch.CudaTensor()
   self.h2 = torch.CudaTensor()
   self.s1 = torch.CudaTensor()
   self.s2 = torch.CudaTensor()
   self.y = torch.CudaTensor()
   self.gradInput = {}
...
function CompactBilinearPooling:conv(x,y)
   ...
   self._conv = self._conv or torch.CudaTensor()
   self._conv:resize(batchSize,1,1,dim*2)
   self.x = signal.fft(self.x_:view(x:size(1),1,1,-1))
   self.y = signal.fft(self.y_:view(y:size(1),1,1,-1))
   self._conv = signal.ifft(self.fft_x:cmul(self.fft_y),1,1,-1)
   --cufft.fft1d(self.x_:view(x:size(1),1,1,-1), self.fft_x)
   --cufft.fft1d(self.y_:view(y:size(1),1,1,-1), self.fft_y)
   --cufft.ifft1d(self.fft_x:cmul(self.fft_y), self._conv)
   return self._conv:resize(batchSize,1,1,dim,2):select(2,1):select(2,1):select(3,1)
end
