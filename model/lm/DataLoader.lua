...
function DataLoader:__init(opt)
  ...
  self.semantic_words = opt.atts
  if self.semantic_words then 
  ...
  local split_ix_train = self.split_ix['train']
  local split_ix_test  = self.split_ix['test']
  self.max_train = #split_ix_train
  self.max_test = #split_ix_test
end

function DataLoader:getTrainSize()
  return self.max_train
end

function DataLoader:getTestSize()
  return self.max_test
end
...
function DataLoader:getBatch(opt)
  ...
  local attrs_batch
  if self.semantic_words then 
    attrs_batch = torch.LongTensor(batch_size, self.top_attrs_k)
  end
...
function DataLoader:getBatch_299(opt)
  local split = utils.getopt(opt, 'split') -- lets require that user passes this in, for safety
  local batch_size = utils.getopt(opt, 'batch_size', 5) -- how many images get returned at one time (to go through CNN)
  local seq_per_img = utils.getopt(opt, 'seq_per_img', 5) -- number of sequences to return per image
  local split_ix = self.split_ix[split]
  ...
  -- pick an index of the datapoint to load next
  local img_batch_raw = torch.ByteTensor(batch_size, 3, 299, 299)
  local label_batch = torch.LongTensor(batch_size * seq_per_img, self.seq_length)
  local max_index = #split_ix
  local wrapped = false
  local infos = {}
