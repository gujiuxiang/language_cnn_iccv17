...
local layer, parent = torch.class('nn.LanguageModel_SC', 'nn.Module')
function layer:__init(opt)
  parent.__init(self)

  -- options for core network
  self.rnn_type = utils.getopt(opt, 'rnn_type', 'BNLSTM')
  ...
  if self.rnn_type == 'BNLSTM' then
    self.core = BNLSTM.BNLSTM(2*self.input_encoding_size, self.vocab_size + 1, self.rnn_size, self.num_layers, dropout, true)
  else
    self.core = LSTM.lstm(2*self.input_encoding_size, self.vocab_size + 1, self.rnn_size, self.num_layers, dropout, true)
  end
  ...
  self.eltwise = nn.CMulTable() -- element-wise multiplication
...
function layer:getModulesList()
  return {self.core, self.lookup_table, self.lookup_table_tc}
end
...
function layer:parameters()
  ...
  local p3,g3 = self.lookup_table_tc:parameters()
...
function layer:sample(imgs, opt)
  ...
  local hidden_out= torch.CudaTensor(self.seq_length, self.rnn_size)
  ...
  for t=1,self.seq_length+2 do
    ...
    if t>=3 then
      hidden_out[t-2] = out[self.num_state][1]
    end
    state = {}
  ...
  return seq, seqLogprobs, hidden_out
end
  ...
  for t=1,self.seq_length+2 do
    ...
    elseif t == 2 then
      ...
      if not can_skip then
        ...
        local lookup_table_to_tensor = torch.CudaTensor(batch_size, t-1)
        for j = 2,t do lookup_table_to_tensor:sub(1,batch_size,j-1,j-1):copy(self.lookup_tables_inputs[j]) end
...
function layer:updateGradInput(input, gradOutput)
  ...
  for t=self.tmax,1,-1 do
    ...
    if t == 1 then
      ...
      for j = 1,batch_size do dlookup_table_out[j] = torch.expand(dimg_text[2][j]:resize(1,dimg_text[2]:size(2)),t-1,dimg_text[2]:size(2))/(t-1) end
...
local crit, parent = torch.class('nn.LanguageModelCriterion_SC', 'nn.Criterion')
