...
function HighwayMLP.rhw(size, num_layers, dropout,bias, f)
  -- size = dimensionality of inputs
  -- num_layers = number of hidden layers (default = 1)
  -- bias = bias for transform gate (default = -2)
  -- f = non-linearity (default = ReLU)
  -- dropout: dropout probability between two layers
  local rhn_inputs = {}
  table.insert(rhn_inputs, nn.Identity()()) -- captions
  table.insert(rhn_inputs, nn.Identity()()) -- s1

  local input =rhn_inputs[1] --bs,16,512
  local state_in = rhn_inputs[2]
  ...
  state_out = inputs[#inputs]
  return nn.gModule({rhn_inputs,state_in},{output})
end
