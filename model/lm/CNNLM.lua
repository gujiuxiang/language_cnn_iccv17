...
local CNNLM = torch.class('CNNLM')

function CNNLM:make_lm_net()
  ...
  local input = nn.Identity()() --bs,16,512
  ...
  if config.non_linearity == 'tanh' then nonlinear_func = cudnn.Tanh
  elseif config.non_linearity == 'sigmoid' then nonlinear_func = cudnn.Sigmoid
  elseif config.non_linearity == 'relu' then nonlinear_func = cudnn.ReLU
  end
...
function CNNLM:make_net(config, vocab_size)
